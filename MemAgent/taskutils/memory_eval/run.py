...
GLM_4_9B_Chat_1M = Config(
    name="GLM-4-9B-Chat-1M",
    ckpt="zai-org/glm-4-9b-chat-1m",
    tp=8,  # tp=1
    method="openai",
    concur=256,
    env=ENV(),
...
def run_ruler_hqa(c):
        ...
        task = [50, 100, 200, 400, 800]#, 1600, 3200, 6400]
        if c.name.startswith("MemoryAgent"):
            task += RULER_HQA_TESTS_OVER_1M
        c.run(task, serve=False, force=False) # skip original serve for vllm deployment due to version incompatibility
...
if __name__ == "__main__":
    ...
    for c in CONFIGS:
        print(f"\n=== Starting vLLM server for {c.name} ===")
        serve_cmd = [
            "vllm", "serve", c.ckpt,
            "--tensor-parallel-size", str(c.tp),
            "--port", SERVE_PORT,
            "--host", "0.0.0.0",
        ]
        print(" ".join(serve_cmd))
        serve_proc = subprocess.Popen(serve_cmd, preexec_fn=os.setsid)

        # Wait for REST API ready
        print("Waiting for vLLM REST API to start...")
        ready = False
        for _ in range(120):  # up to ~6 min
            p = subprocess.run(
                ["curl", "-s", f"http://127.0.0.1:{SERVE_PORT}/v1/models"],
                capture_output=True,
            )
            if p.returncode == 0 and c.model.split("/")[-1] in p.stdout.decode():
                print("✅ vLLM serve ready.")
                ready = True
                break
            time.sleep(3)
        if not ready:
            print("❌ Failed to start vLLM serve, skipping.")
            os.killpg(os.getpgid(serve_proc.pid), signal.SIGTERM)
            continue

        # Run tests without starting its own serve()
        try:
            run_ruler_hqa(c)
            run_ood_tasks(c)
        finally:
            print(f"🧹 Stopping vLLM serve for {c.name}...")
            os.killpg(os.getpgid(serve_proc.pid), signal.SIGTERM)
            try:
